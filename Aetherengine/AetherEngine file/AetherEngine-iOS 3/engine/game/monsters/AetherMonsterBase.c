...
void aether_monster_init(aether_monster_t*m,u32 id,const char*n,aether_vec3_t p,f32 hp,f32 speed){if(!m)return;memset(m,0,sizeof*m);m->id=id;m->position=p;m->max_health=hp;m->health=hp;m->speed=speed;m->perception_range=1024;m->attack_range=64;m->attack_damage=10.0f;m->attack_windup=0.2f;m->friendly=false;m->state=AETHER_MONSTER_IDLE;m->alive=true;if(n){strncpy(m->classname,n,63);m->classname[63]=0;}}
