...
vertex FxVertexOut aether_fx_vertex(FxVertexIn in [[stage_in]],
                                    constant Uniforms &U [[buffer(1)]]) {
    FxVertexOut out;
    float4 world = U.model * float4(in.position, 1.0);
    out.position = U.proj * U.view * world;
...
fragment float4 aether_fx_fragment(FxVertexOut in [[stage_in]]) {
    return in.color;
}
...
fragment float4 aether_model_fragment(MdlVertexOut in [[stage_in]],
                                       constant Uniforms &U [[buffer(1)]],
                                       texture2d<float> model_tex [[texture(0)]],
                                       sampler samp [[sampler(0)]]) {
    float3 N = normalize(in.normal);
    ...
    if (U.use_texture > 0.5) {
        float4 texel = model_tex.sample(samp, in.uv);
        if (texel.a > 0.01) base = texel.rgb;
    }
