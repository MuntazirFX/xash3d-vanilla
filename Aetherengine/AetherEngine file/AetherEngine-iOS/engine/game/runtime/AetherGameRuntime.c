...
aether_result_t aether_game_runtime_add_monster(aether_game_runtime_t *g,
    ...
    u32 slot = g->monster_count++;
    g->monsters[slot] = *m;
    aether_ai_init(&g->monster_ai[slot]);
    { const aether_npc_profile_t *p = aether_npc_profile_for_classname(m->classname);
      if (p) { g->monsters[slot].attack_damage = p->attack_damage; g->monsters[slot].attack_windup = p->attack_windup; g->monsters[slot].friendly = p->friendly; } }
    aether_ai_set_ranges(&g->monster_ai[slot], m->perception_range, m->attack_range, m->perception_range * 1.35f);
    g->monster_ai[slot].attack_cooldown = m->attack_cooldown > 0.0f ? m->attack_cooldown : 1.0f;
    return AETHER_OK;
}
...
aether_result_t aether_game_runtime_spawn_monster(aether_game_runtime_t *g,
    ...
    if (!g || !classname) return AETHER_ERR_INVALID_ARG;
    if (!aether_npc_profile_for_classname(classname)) return AETHER_ERR_NOT_FOUND;
    aether_monster_t m;
    aether_npc_spawn(&m, id, classname, position);
    return aether_game_runtime_add_monster(g, &m);
}
...
void aether_game_runtime_update(aether_game_runtime_t *g,
    ...
    for (u32 i = 0; i < g->monster_count; ++i) {
        ...
        aether_ai_update(&g->monster_ai[i], m, 1u, target, dt);
        if (!m->friendly && aether_ai_can_attack(&g->monster_ai[i]) &&
            aether_npc_can_attack(m, g->time)) {
            (void)aether_ai_take_attack(&g->monster_ai[i]);
            const aether_npc_profile_t *profile = aether_npc_profile_for_classname(m->classname);
            f32 dmg = aether_npc_attack_damage(m);
            if (profile && profile->attack_type == AETHER_NPC_ATTACK_MELEE &&
                strcmp(m->classname, "monster_headcrab") == 0) {
                dmg = aether_headcrab_attack(m);
            } else {
                aether_npc_mark_attack(m);
            }
            aether_damage_t d = {
                dmg, profile && profile->attack_type == AETHER_NPC_ATTACK_RANGED ? 0.65f : 0.5f,
                profile && profile->attack_type == AETHER_NPC_ATTACK_RANGED ? AETHER_DAMAGE_BULLET : AETHER_DAMAGE_MELEE,
                m->position, (aether_vec3_t){0,0,0}, m
            };
            aether_game_runtime_damage_player(g, &d);
            emit_event(g, AETHER_COMBAT_PLAYER_HIT, g->active_weapon, m->id, dmg);
