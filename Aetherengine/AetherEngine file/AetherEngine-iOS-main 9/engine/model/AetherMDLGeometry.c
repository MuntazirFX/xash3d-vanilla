    ...
    *out_mesh=NULL;
    ...
    for(i32 bp=0;bp<body_count;++bp){
        ...
        for(i32 mi=0;mi<b->num_models;++mi){
            ...
            if(!range_ok(mo,MDL_MODEL_SIZE,size))break;
            const mdl_model_disk_t *m=(const mdl_model_disk_t*)(raw+mo);
            if(m->num_verts<=0||m->num_norms<=0||m->vert_info_index<=0||m->norm_info_index<=0||m->mesh_index<=0)continue;
            if(!range_ok((u32)m->vert_info_index,(u64)m->num_verts,size) ||
               !range_ok((u32)m->norm_info_index,(u64)m->num_norms,size)) continue;
            const u8 *verts=raw+(u32)m->vert_info_index; /* fixed below via model vertex index */
            /* In studio models vert_info_index is the bone byte table; vertex
               positions are stored at vertindex immediately before it in the
               model descriptor. See explicit descriptor offsets below. */
            i32 vert_index=rd_i32(raw+mo+80);
            if(vert_index<=0||!range_ok((u32)vert_index,(u64)m->num_verts*MDL_VERTEX_SIZE,size))continue;
            if(!range_ok((u32)m->vert_info_index,(u64)m->num_verts,size))continue;
            const u8 *vpos=raw+(u32)vert_index;
            const u8 *vbone=verts;
            i32 norm_index=rd_i32(raw+mo+92);
            if(norm_index<=0||!range_ok((u32)norm_index,(u64)m->num_norms*MDL_VERTEX_SIZE,size))continue;
            const u8 *npos=raw+(u32)norm_index;
