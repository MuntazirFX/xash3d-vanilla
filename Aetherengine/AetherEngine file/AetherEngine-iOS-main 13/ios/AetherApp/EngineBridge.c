...
#include "../../engine/game/AetherPickup.h"
#include "../../engine/client/view/AetherViewModel.h"
...
static aether_mdl_t             *g_mdl          = NULL;
static aether_mdl_sequence_set_t g_mdl_sequences = {0};
static aether_mdl_animator_t    g_mdl_animator;
static aether_mdl_pose_t        g_mdl_pose;
static bool                     g_mdl_animator_ready = false;
static f32                      *g_mdl_skinned_pos = NULL;
static f32                      *g_mdl_skinned_norm = NULL;
static u32                       g_mdl_skinned_capacity = 0;
...
void engine_init(const char *base_path, const char *asset_path) {
    ...
    aether_game_runtime_init(&g_runtime);
...
void engine_shutdown(void) {
    ...
    if (g_mdl_skinned_pos) { free(g_mdl_skinned_pos); g_mdl_skinned_pos = NULL; }
    if (g_mdl_skinned_norm) { free(g_mdl_skinned_norm); g_mdl_skinned_norm = NULL; }
    g_mdl_skinned_capacity = 0;
    g_loaded_viewmodel[0] = '\0';
    aether_mdl_pose_free(&g_mdl_pose);
    aether_mdl_sequences_free(&g_mdl_sequences);
    g_mdl_animator_ready = false;
    if (g_mdl) { aether_mdl_free(g_mdl); g_mdl = NULL; }
    if (g_atlas)        { aether_texture_atlas_free(g_atlas); g_atlas = NULL; }
    if (g_collision)    { if (g_runtime_ready) aether_game_runtime_set_collision(&g_runtime, NULL); aether_collision_free(g_collision); g_collision = NULL; }
    if (g_runtime_ready) { aether_game_runtime_shutdown(&g_runtime); g_runtime_ready = false; }
...
void engine_launch_game(const char *game_dir) {
    ...
    if (g_runtime_ready) {
        (void)engine_game_refresh_viewmodel();
    }
}
...
void engine_player_spawn_at_mesh_center(void) {
    if (!g_active_mesh || !g_runtime_ready) return;
    aether_vec3_t c = {
        g_active_mesh->bounds_center[0],
        g_active_mesh->bounds_center[1],
        g_active_mesh->bounds_center[2] + 50.0f
    };
    (void)aether_game_runtime_start(&g_runtime, c, 0.0f);
    g_player = g_runtime.player;
    aether_log(AETHER_LOG_INFO, "player", "spawned at (%.1f, %.1f, %.1f)", c.x, c.y, c.z);
}

void engine_player_tick(float dt) {
    if (!g_input || !g_runtime_ready) return;
    aether_input_begin_frame(g_input);
    const aether_input_state_t *st = aether_input_state(g_input);
    aether_game_runtime_update(&g_runtime, st, dt);
    g_player = g_runtime.player;
    aether_input_end_frame(g_input);
}

void engine_game_tick(float dt) {
    engine_player_tick(dt);
    (void)engine_game_refresh_viewmodel();
}
int engine_game_state(void) { return g_runtime_ready ? (int)aether_game_runtime_state(&g_runtime) : 0; }
float engine_game_health(void) { const aether_player_health_t *h = aether_game_runtime_health(&g_runtime); return h ? h->health : 0.0f; }
float engine_game_armor(void) { const aether_player_health_t *h = aether_game_runtime_health(&g_runtime); return h ? h->armor : 0.0f; }
int engine_game_weapon_id(void) { const aether_weapon_t *w = aether_game_runtime_weapon(&g_runtime); return w ? (int)w->id : 0; }
int engine_game_weapon_clip(void) { const aether_weapon_t *w = aether_game_runtime_weapon(&g_runtime); return w ? w->clip : 0; }
int engine_game_weapon_reserve(void) { const aether_weapon_t *w = aether_game_runtime_weapon(&g_runtime); return w ? w->ammo : 0; }
unsigned int engine_game_monster_count(void) { return g_runtime_ready ? aether_game_runtime_monster_count(&g_runtime) : 0; }
unsigned int engine_game_monsters_alive(void) {
    if (!g_runtime_ready) return 0;
    unsigned int n = 0;
    for (u32 i = 0; i < g_runtime.monster_count; ++i) if (g_runtime.monsters[i].alive) ++n;
    return n;
}
unsigned int engine_game_shots_fired(void) { return g_runtime_ready ? g_runtime.shots_fired : 0; }
unsigned int engine_game_shots_hit(void) { return g_runtime_ready ? g_runtime.shots_hit : 0; }
unsigned int engine_game_monsters_killed(void) { return g_runtime_ready ? g_runtime.monsters_killed : 0; }
unsigned int engine_game_pickup_count(void) { return g_runtime_ready ? aether_game_runtime_pickup_count(&g_runtime) : 0; }
const char *engine_game_weapon_name(void) { return g_runtime_ready ? aether_weapon_name(g_runtime.active_weapon) : "None"; }
float engine_game_damage_flash(void) { return g_runtime_ready ? g_runtime.hud.damage_flash : 0.0f; }
const char *engine_game_hud_message(void) { return g_runtime_ready ? g_runtime.hud.message : ""; }
const char *engine_game_viewmodel_path(void) {
    return g_runtime_ready ? aether_combat_presentation_viewmodel(&g_runtime.presentation) : "";
}

int engine_game_refresh_viewmodel(void) {
    if (!g_runtime_ready || !g_fs) return 0;
    const char *path = aether_viewmodel_path(&g_runtime.viewmodel);
    if (!path || !path[0]) return 0;
    if (aether_str_eq(g_loaded_viewmodel, path) && g_mdl_mesh) return 1;
    if (!engine_mdl_mesh_build(path)) {
        g_loaded_viewmodel[0] = '\0';
        return 0;
    }
    snprintf(g_loaded_viewmodel, sizeof g_loaded_viewmodel, "%s", path);
    return 1;
}

void engine_game_viewmodel_offset(float out_xyz[3]) {
    if (!out_xyz) return;
    if (!g_runtime_ready) { out_xyz[0]=out_xyz[1]=out_xyz[2]=0.0f; return; }
    aether_viewmodel_offset(&g_runtime.viewmodel, out_xyz);
}
float engine_game_viewmodel_scale(void) {
    return g_runtime_ready ? aether_viewmodel_scale(&g_runtime.viewmodel) : 1.0f;
}
bool engine_game_viewmodel_visible(void) {
    return g_runtime_ready ? aether_viewmodel_visible(&g_runtime.viewmodel) : false;
}
float engine_game_muzzle_flash(void) {
    return g_runtime_ready ? aether_combat_presentation_muzzle_flash(&g_runtime.presentation) : 0.0f;
}
float engine_game_recoil_pitch(void) {
    return g_runtime_ready ? aether_combat_presentation_recoil_pitch(&g_runtime.presentation) : 0.0f;
}
float engine_game_recoil_yaw(void) {
    return g_runtime_ready ? aether_combat_presentation_recoil_yaw(&g_runtime.presentation) : 0.0f;
}
const char *engine_game_presentation_event(void) {
    return g_runtime_ready ? aether_combat_presentation_event_text(&g_runtime.presentation) : "";
}
int engine_game_last_event_type(void) { return g_runtime_ready ? (int)g_runtime.last_event.type : 0; }
int engine_game_last_event_weapon(void) { return g_runtime_ready ? (int)g_runtime.last_event.weapon : 0; }
unsigned int engine_game_last_event_target(void) { return g_runtime_ready ? g_runtime.last_event.target_id : 0; }
float engine_game_last_event_damage(void) { return g_runtime_ready ? g_runtime.last_event.damage : 0.0f; }
void engine_game_clear_last_event(void) { if (g_runtime_ready) aether_game_runtime_clear_event(&g_runtime); }
...
void engine_player_set_position(float x, float y, float z) {
    ...
    if (g_runtime_ready) g_runtime.player = g_player;
}
void  engine_player_set_angles(float y, float p) { g_player.yaw = y; g_player.pitch = p; if (g_runtime_ready) { g_runtime.player.yaw = y; g_runtime.player.pitch = p; } }
...
int engine_bsp_mesh_build(const char *vp) {
    ...
    if (g_collision) {
        ...
        if (g_runtime_ready) aether_game_runtime_set_collision(&g_runtime, g_collision);
    }

    /* Parse entities and seed the first gameplay runtime. */
    g_entities = aether_entity_list_from_bsp(b);
    if (g_entities) {
        aether_entity_list_dump(g_entities);
        if (g_runtime_ready) {
            const aether_entity_t *spawn = aether_entity_find_first(g_entities, AETHER_ENTITY_PLAYER_START);
            aether_vec3_t pos = {0, 0, 0};
            f32 yaw = 0.0f;
            if (spawn) {
                pos = (aether_vec3_t){spawn->origin[0], spawn->origin[1], spawn->origin[2]};
                yaw = spawn->angles[1] * 3.14159265359f / 180.0f;
            } else if (g_active_mesh) {
                pos = (aether_vec3_t){g_active_mesh->bounds_center[0], g_active_mesh->bounds_center[1], g_active_mesh->bounds_center[2] + 50.0f};
            }
            (void)aether_game_runtime_start(&g_runtime, pos, yaw);
            for (u32 i = 0; i < aether_entity_list_count(g_entities); ++i) {
                const aether_entity_t *e = aether_entity_at(g_entities, i);
                if (e && strcmp(e->classname, "monster_headcrab") == 0) {
                    aether_monster_t m;
                    aether_headcrab_init(&m, i + 1,
                        (aether_vec3_t){e->origin[0], e->origin[1], e->origin[2]});
                    (void)aether_game_runtime_add_monster(&g_runtime, &m);
                }
            }
            for (u32 i = 0; i < aether_entity_list_count(g_entities); ++i) {
                const aether_entity_t *e = aether_entity_at(g_entities, i);
                if (!e) continue;
                aether_pickup_t p;
                aether_vec3_t pos = { e->origin[0], e->origin[1], e->origin[2] };
                bool add = true;
                if (strcmp(e->classname, "weapon_9mmhandgun") == 0)
                    aether_pickup_init(&p, i + 1, e->classname, pos, AETHER_PICKUP_WEAPON, AETHER_WEAPON_GLOCK, NULL, 0, 0);
                else if (strcmp(e->classname, "weapon_crowbar") == 0)
                    aether_pickup_init(&p, i + 1, e->classname, pos, AETHER_PICKUP_WEAPON, AETHER_WEAPON_CROWBAR, NULL, 0, 0);
                else if (strcmp(e->classname, "item_healthkit") == 0)
                    aether_pickup_init(&p, i + 1, e->classname, pos, AETHER_PICKUP_HEALTH, AETHER_WEAPON_NONE, NULL, 25, 100);
                else if (strcmp(e->classname, "item_battery") == 0)
                    aether_pickup_init(&p, i + 1, e->classname, pos, AETHER_PICKUP_ARMOR, AETHER_WEAPON_NONE, NULL, 15, 100);
                else if (strcmp(e->classname, "item_suit") == 0)
                    aether_pickup_init(&p, i + 1, e->classname, pos, AETHER_PICKUP_SUIT, AETHER_WEAPON_NONE, NULL, 1, 1);
                else if (strcmp(e->classname, "ammo_9mmclip") == 0)
                    aether_pickup_init(&p, i + 1, e->classname, pos, AETHER_PICKUP_AMMO, AETHER_WEAPON_NONE, "9mm", 17, 250);
                else if (strcmp(e->classname, "ammo_9mmAR") == 0)
                    aether_pickup_init(&p, i + 1, e->classname, pos, AETHER_PICKUP_AMMO, AETHER_WEAPON_NONE, "9mm", 50, 250);
                else if (strcmp(e->classname, "ammo_buckshot") == 0)
                    aether_pickup_init(&p, i + 1, e->classname, pos, AETHER_PICKUP_AMMO, AETHER_WEAPON_NONE, "buckshot", 12, 125);
                else add = false;
                if (add) (void)aether_game_runtime_add_pickup(&g_runtime, &p);
            }
            g_player = g_runtime.player;
        }
    }
...
int engine_mdl_mesh_build(const char *mdl_vpath) {
    ...
    if (r != AETHER_OK || !mesh) { aether_mdl_free(mdl); return 0; }

    aether_mdl_sequence_set_t sequences = {0};
    (void)aether_mdl_sequences_parse(mdl, &sequences);
    aether_mdl_pose_init(&g_mdl_pose);
    aether_mdl_animator_init(&g_mdl_animator);
    aether_mdl_animator_bind(&g_mdl_animator, &sequences);
    if (sequences.count > 0) {
        (void)aether_mdl_animator_play(&g_mdl_animator, 0, true, 1.0f);
        g_mdl_animator_ready = true;
    }
    if (g_mdl) aether_mdl_free(g_mdl);
    aether_mdl_sequences_free(&g_mdl_sequences);
    g_mdl = mdl;
    g_mdl_sequences = sequences;
    g_mdl_mesh = mesh;
...
int engine_mdl_mesh_copy_uvs(float *out, int max_floats) {
    if (!g_mdl_mesh || !g_mdl_mesh->uvs || !out || max_floats <= 0) return 0;
    int n = (int)(g_mdl_mesh->vertex_count * 2);
    if (n > max_floats) n = max_floats;
    memcpy(out, g_mdl_mesh->uvs, (size_t)n * sizeof(f32));
    return n;
}
int engine_mdl_mesh_copy_bones(unsigned char *out, int max_bytes) {
    if (!g_mdl_mesh || !g_mdl_mesh->bone_indices || !out || max_bytes <= 0) return 0;
    int n = (int)g_mdl_mesh->vertex_count;
    if (n > max_bytes) n = max_bytes;
    memcpy(out, g_mdl_mesh->bone_indices, (size_t)n);
    return n;
}
int engine_mdl_mesh_texture_width(void) { return g_mdl_mesh ? (int)g_mdl_mesh->texture_width : 0; }
int engine_mdl_mesh_texture_height(void) { return g_mdl_mesh ? (int)g_mdl_mesh->texture_height : 0; }
int engine_mdl_mesh_copy_texture_rgba(unsigned char *out, int max_bytes) {
    if (!g_mdl_mesh || !g_mdl_mesh->texture_rgba || !out || max_bytes <= 0) return 0;
    int n = (int)(g_mdl_mesh->texture_width * g_mdl_mesh->texture_height * 4u);
    if (n > max_bytes) n = max_bytes;
    memcpy(out, g_mdl_mesh->texture_rgba, (size_t)n);
    return n;
}
...
/* ---------- MDL skeletal animation (STEP 16D) ---------- */
int engine_mdl_animation_sequence_count(void) { return (int)g_mdl_sequences.count; }
int engine_mdl_animation_play(int sequence, bool looping, float playback_rate) {
    if (!g_mdl_animator_ready) return 0;
    return aether_mdl_animator_play(&g_mdl_animator, sequence, looping, playback_rate) ? 1 : 0;
}
int engine_mdl_animation_play_name(const char *label, bool looping, float playback_rate) {
    if (!g_mdl_animator_ready || !label) return 0;
    const aether_mdl_sequence_t *s = aether_mdl_sequence_find(&g_mdl_sequences, label);
    if (!s) return 0;
    int index = (int)(s - g_mdl_sequences.items);
    return engine_mdl_animation_play(index, looping, playback_rate);
}
void engine_mdl_animation_stop(void) { if (g_mdl_animator_ready) aether_mdl_animator_stop(&g_mdl_animator); }
void engine_mdl_animation_tick(float dt) {
    if (!g_mdl_animator_ready || !g_mdl || !g_mdl_mesh) return;
    aether_mdl_animator_update(&g_mdl_animator, dt);
    const aether_mdl_sequence_t *seq = aether_mdl_animator_current(&g_mdl_animator);
    if (!seq) return;
    if (aether_mdl_pose_build(g_mdl, g_mdl_animator.sequence, aether_mdl_animator_frame(&g_mdl_animator),
                              g_mdl_animator.blend[0], g_mdl_animator.blend[1], &g_mdl_pose) != AETHER_OK) return;
    u32 n = g_mdl_mesh->vertex_count;
    if (n > g_mdl_skinned_capacity) {
        size_t bytes = (size_t)n * 3u * sizeof(f32);
        f32 *np = (f32*)malloc(bytes);
        f32 *nn = (f32*)malloc(bytes);
        if (!np || !nn) { free(np); free(nn); return; }
        if (g_mdl_skinned_pos && g_mdl_skinned_capacity)
            memcpy(np, g_mdl_skinned_pos, (size_t)g_mdl_skinned_capacity * 3u * sizeof(f32));
        if (g_mdl_skinned_norm && g_mdl_skinned_capacity)
            memcpy(nn, g_mdl_skinned_norm, (size_t)g_mdl_skinned_capacity * 3u * sizeof(f32));
        free(g_mdl_skinned_pos); free(g_mdl_skinned_norm);
        g_mdl_skinned_pos=np; g_mdl_skinned_norm=nn; g_mdl_skinned_capacity=n;
    }
    (void)aether_mdl_pose_skin_mesh(g_mdl_mesh, &g_mdl_pose, g_mdl_skinned_pos, g_mdl_skinned_norm, n);
}
int engine_mdl_animation_sequence(void) { return g_mdl_animator_ready ? g_mdl_animator.sequence : -1; }
float engine_mdl_animation_frame(void) { return g_mdl_animator_ready ? aether_mdl_animator_frame(&g_mdl_animator) : 0.0f; }
int engine_mdl_mesh_copy_skinned_positions(float *out, int max_floats) {
    if (!g_mdl_skinned_pos || !g_mdl_mesh || !out || max_floats <= 0) return 0;
    int n=(int)(g_mdl_mesh->vertex_count*3u); if(n>max_floats)n=max_floats; memcpy(out,g_mdl_skinned_pos,(size_t)n*sizeof(f32)); return n;
}
int engine_mdl_mesh_copy_skinned_normals(float *out, int max_floats) {
    if (!g_mdl_skinned_norm || !g_mdl_mesh || !out || max_floats <= 0) return 0;
    int n=(int)(g_mdl_mesh->vertex_count*3u); if(n>max_floats)n=max_floats; memcpy(out,g_mdl_skinned_norm,(size_t)n*sizeof(f32)); return n;
}
