...
/* GoldSrc mstudioseqdesc_t (v10) layout. */
#define SEQ_FPS 32u
#define SEQ_FLAGS 36u
#define SEQ_ACTIVITY 40u
#define SEQ_ACTIVITY_WEIGHT 44u
...
aether_result_t aether_mdl_sequences_parse(const aether_mdl_t *mdl,
    ...
    for (i32 i = 0; i < count; ++i) {
        ...
        items[i].fps = rd_f32(p + SEQ_FPS);
        items[i].flags = rd_i32(p + SEQ_FLAGS);
        items[i].activity = rd_i32(p + SEQ_ACTIVITY);
        items[i].activity_weight = rd_i32(p + SEQ_ACTIVITY_WEIGHT);
        items[i].num_events = rd_i32(p + SEQ_NUMEVENTS);
        items[i].event_index = rd_i32(p + SEQ_EVENTINDEX);
        items[i].num_frames = rd_i32(p + SEQ_NUMFRAMES);
        items[i].num_blends = rd_i32(p + SEQ_NUMBLENDS);
        items[i].motion_type = rd_i32(p + SEQ_MOTIONTYPE);
        items[i].motion_bone = rd_i32(p + SEQ_MOTIONBONE);
        for (int k = 0; k < 3; ++k) items[i].linearmovement[k] = rd_f32(p + SEQ_LINEAR + k * 4);
        for (int k = 0; k < 3; ++k) items[i].bbmin[k] = rd_f32(p + SEQ_BBMIN + k * 4);
        for (int k = 0; k < 3; ++k) items[i].bbmax[k] = rd_f32(p + SEQ_BBMAX + k * 4);
        items[i].blend_type[0] = rd_i32(p + SEQ_BLENDTYPE);
        items[i].blend_type[1] = rd_i32(p + SEQ_BLENDTYPE + 4);
        items[i].blend_start[0] = rd_f32(p + SEQ_BLENDSTART);
        items[i].blend_start[1] = rd_f32(p + SEQ_BLENDSTART + 4);
        items[i].blend_end[0] = rd_f32(p + SEQ_BLENDEND);
        items[i].blend_end[1] = rd_f32(p + SEQ_BLENDEND + 4);
        items[i].blend_parent = rd_i32(p + SEQ_BLENDPARENT);
        items[i].anim_index = rd_i32(p + SEQ_ANIMINDEX);
        items[i].seq_group = rd_i32(p + SEQ_GROUP);
