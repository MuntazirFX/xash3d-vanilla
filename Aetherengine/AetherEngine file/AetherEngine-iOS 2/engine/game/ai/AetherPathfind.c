...
bool aetherpathfind_add_link(aether_pathfind_state_t *s, u32 from_id, u32 to_id, f32 cost) {
    ...
    /* Links are stored contiguously per source. This small graph builder only
       accepts links in source-node order, which keeps runtime traversal cheap. */
    if (s->nodes[from].link_count == 0) s->nodes[from].first_link = (u16)s->link_count;
    else if ((u32)s->nodes[from].first_link + s->nodes[from].link_count != s->link_count) return false;
    s->links[s->link_count++] = (aether_path_link_t){(u16)to, cost};
...
u32 aetherpathfind_find(const aether_pathfind_state_t *s, u32 start_id, u32 goal_id,
    ...
    for (;;) {
        ...
        const aether_path_node_t *n=&s->nodes[current];
        for (u32 j=0;j<n->link_count;++j) {
            const u32 li=(u32)n->first_link+j;
            if(li>=s->link_count) return 0;
