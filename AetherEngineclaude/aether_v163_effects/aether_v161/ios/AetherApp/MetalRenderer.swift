...
final class MetalRenderer: NSObject, MTKViewDelegate {
    ...
    // Combat FX pipeline: transient billboards/beam strips generated from
    // engine FX events. The game data remains external to the IPA.
    ...
    init?(mtkView: MTKView) {
        ...
        buildFXPipeline(mtkView: mtkView)
        buildDepthState()
        buildSampler()
    ...
    // MARK: - Combat FX pipeline
    private func buildFXPipeline(mtkView: MTKView) {
        guard let lib = device.makeDefaultLibrary(),
              let vfn = lib.makeFunction(name: "aether_fx_vertex"),
              let ffn = lib.makeFunction(name: "aether_fx_fragment") else {
            print("[MetalRenderer] FX shader funcs missing")
            return
        }
        ...
        do { fxPipeline = try device.makeRenderPipelineState(descriptor: d) }
        catch { print("[MetalRenderer] FX pipeline error: \(error)") }

        let depth = MTLDepthStencilDescriptor()
        depth.depthCompareFunction = .lessEqual
        depth.isDepthWriteEnabled = false
        fxDepthState = device.makeDepthStencilState(descriptor: depth)
    }
    ...
    private func appendFXQuad(_ vertices: inout [FXVertex],
        ...
        vertices.append(FXVertex(position: center - right * halfWidth - up * halfHeight, color: color))
        vertices.append(FXVertex(position: center + right * halfWidth - up * halfHeight, color: color))
        vertices.append(FXVertex(position: center + right * halfWidth + up * halfHeight, color: color))
        vertices.append(FXVertex(position: center - right * halfWidth + up * halfHeight, color: color))
    ...
    private func appendFXTracer(_ vertices: inout [FXVertex],
                                _ indices: inout [UInt32],
                                start: simd_float3,
                                end: simd_float3,
        ...
        let dir = axis / length
        var side = simd_cross(dir, cameraForward)
        if simd_length_squared(side) < 0.0001 {
            side = simd_cross(dir, simd_float3(0, 0, 1))
        }
        side = simd_normalize(side) * 0.65
        let base = UInt32(vertices.count)
        vertices.append(FXVertex(position: start - side, color: color))
        vertices.append(FXVertex(position: start + side, color: color))
        vertices.append(FXVertex(position: end + side, color: color))
        vertices.append(FXVertex(position: end - side, color: color))
        indices += [base, base + 1, base + 2, base, base + 2, base + 3]
    }

    private func drawCombatFX(_ enc: MTLRenderCommandEncoder,
                              eye: simd_float3,
                              forward: simd_float3,
                              view: simd_float4x4,
                              proj: simd_float4x4) {
        guard let pipeline = fxPipeline else { return }
        let count = Int(engine_game_fx_count())
        guard count > 0 else { return }

        let f = simd_normalize(forward)
        var right = simd_cross(f, simd_float3(0, 0, 1))
        if simd_length_squared(right) < 0.0001 { right = simd_float3(0, 1, 0) }
        right = simd_normalize(right)
        let up = simd_normalize(simd_cross(right, f))

        var vertices: [FXVertex] = []
        var indices: [UInt32] = []
        vertices.reserveCapacity(count * 4)
        indices.reserveCapacity(count * 6)

        for i in 0..<count {
            let type = engine_game_fx_type(UInt32(i))
            var origin = [Float](repeating: 0, count: 3)
            var end = [Float](repeating: 0, count: 3)
            engine_game_fx_origin(UInt32(i), &origin)
            engine_game_fx_end(UInt32(i), &end)
            let age = engine_game_fx_age(UInt32(i))
            let life = max(engine_game_fx_lifetime(UInt32(i)), 0.001)
            let intensity = max(0, engine_game_fx_intensity(UInt32(i)))
            let alpha = max(0, 1.0 - age / life) * min(1.0, intensity)
            guard alpha > 0.01 else { continue }

            let o = simd_float3(origin[0], origin[1], origin[2])
            switch type {
            case 1: // muzzle
                appendFXQuad(&vertices, &indices, center: o + f * 1.5,
                             right: right, up: up,
                             halfWidth: 4.0 * intensity, halfHeight: 4.0 * intensity,
                             color: simd_float4(1.0, 0.78, 0.32, alpha))
            case 2: // tracer
                let e = simd_float3(end[0], end[1], end[2])
                appendFXTracer(&vertices, &indices, start: o, end: e,
                               cameraForward: f,
                               color: simd_float4(1.0, 0.92, 0.62, alpha))
            case 3: // impact
                appendFXQuad(&vertices, &indices, center: o,
                             right: right, up: up,
                             halfWidth: 2.5 * intensity, halfHeight: 2.5 * intensity,
                             color: simd_float4(1.0, 0.56, 0.22, alpha))
            default:
                break
            }
        }
        ...
        guard !vertices.isEmpty, !indices.isEmpty,
              let vb = device.makeBuffer(bytes: vertices,
                                          length: vertices.count * MemoryLayout<FXVertex>.stride,
        ...
        var U = Uniforms(model: matrix_identity_float4x4,
                         view: view, proj: proj,
                         lightDir: simd_float3(0, 0, 1), pad0: 0,
                         baseColor: simd_float4(1, 1, 1, 1),
                         useTexture: 0, pad1: 0, pad2: 0, pad3: 0)
    ...
    func draw(in view: MTKView) {
        ...
        // ---- Combat FX ----
        drawCombatFX(enc, eye: eyeV, forward: fwdV, view: viewMat, proj: projMat)
